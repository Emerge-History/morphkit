...
function _loadFile(root, file) {
    ...
    try {
        ...
        logger.error(configCache[id] ? configCache[id].indexOf("REF") : "");
        if (!configCache[id]
            || configCache[id].stamp != stat.mtime
            || configCache[id].indexOf("REF") >= 0) {
    ...
    try {
        ...
        for (var i = 0; i < _files.length; i++) {
            if (fs.statSync(path.join(folder, _files[i])).isFile()) {
                if (_files[i].toLowerCase().indexOf("default.config.js") >= 0) {
                    files.unshift(_files[i]);
                } else {
                    files.push(_files[i]);
                }
            }
